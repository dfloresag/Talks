...
- **Parameters**: 4 Fixed Effects ( $\beta_k$ )  and 1 Random intercept ( $\sigma_1$ )
...
```
### Boxplots of $\widehat{\sigma}_1 and $\widehat{\sigma}_1$
...
th_dta %>% 
  ...
  facet_grid(s_1_fac~mth+n_g_fac, scales = "free_y", labeller = label_parsed)+
...
```{r, fig=="center"}
th_dta %>% 
  filter(parameter == "s.1", n_i>2, s_1==0.2 , mth %in% c("lpml", "rb-lpml"), cns==3) %>%
  ggplot() +
  geom_boxplot(mapping = aes(x = factor(n_i), y = value, fill = mth))+
  facet_grid(s_1_fac~mth+n_g_fac, scales = "free_y", labeller = label_parsed)+
  xlab(label = expression(n[i])) +
...
### `r icon::fa("thumbs-down")` Can be **unstable** in **extreme Cases**, e.g.

- **Cluster Size** is **too small** (e.g. $n_i = 2$) and
